...
def ppdata(table, stop, route, fo):
    ...
    if fo == 'xml':
        ...
        #open json file         print "OPEN FILE: " + json_output
        ...
        if table in ['i', "I"]:
            for i in data["results"]:
                print "From: " + i["origin"]
                for j in i["stops"]:
                    print str(j["stopid"]) + "\t" + j["shortname"] + ", "+ j["fullname"]
...
def req(table, stop, route, fo):
    ...
    if table == "t":
        ...
        host = "www.dublinked.ie"
        header = "http://"+user+":"+password+"@"
        rtquery = urllib.urlencode({'routeid': route, 'operator': 'bac', 'format': fo})
        #request url
        request = header + host + rturl + rtquery
        ...
        host = "www.dublinked.ie"
        header = "http://"+user+":"+password+"@"
        rtquery = urllib.urlencode({'operator': 'bac', 'format': fo})
        #request url
        request = header + host + rturl + rtquery
